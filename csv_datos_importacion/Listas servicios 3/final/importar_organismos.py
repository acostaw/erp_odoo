...
for linea in archivo_lista:
	...
	linea_separada = linea.split(',') if linea and linea not in ['id,name',','] else False
	if linea_separada:
		try:
			tiene_modulo_id = False
			...
			if not objeto_existente:
				...
				data_objeto_creado
				objeto_creado = self.env['intn.organismos'].create(data_objeto_creado)
				data = {
			...
			print('LINEA FALLIDA')
			lineas_fallidas = open('organismos_fallidos.csv','a')
