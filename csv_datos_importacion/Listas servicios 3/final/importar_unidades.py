...
for linea in archivo_lista:
	...
	linea_separada = linea.split(',') if linea and linea not in ['id,name,organismo_id/id',',,'] else False
	if linea_separada:
		try:
			tiene_modulo_id = False
			...
			if not objeto_existente:
				data_objeto_creado={
					...
					'organismo_id':get_object_if_exists(linea_separada[2]).id if get_object_if_exists(linea_separada[2]) else False,
					}
				data_objeto_creado
				objeto_creado = self.env['intn.unidades'].create(data_objeto_creado)
				data = {
			...
			print('LINEA FALLIDA')
			lineas_fallidas = open('unidades_fallidos.csv','a')
